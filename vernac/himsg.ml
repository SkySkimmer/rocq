...
| flags :: rem ->
  ...
  if equal then
    ...
    let pr = function
    | None -> str "??"
    | Some c -> Ppconstr.pr_lconstr_expr ~flags:(Ppconstr.of_combined_flags flags) env sigma c
    in
    pr ct1, pr ct2

let explicit_flags () =
  let open PrintingFlags.Combined in
  ...
  let flags = PrintingFlags.Combined.current() in
